...
class ImageListBatchGenerator(object):
	...
	def next_batch(self,batch_size,
						pos_percent = .3):
		batch = np.zeros((batch_size,self.window_radius * 2 + 1,self.window_radius * 2 + 1,self.window_radius * 2 + 1,1))
	...
	def pos_patch(self,curr_image_gold):
		...
		curr_patch = curr_image_gold.image[x - self.window_radius : x + self.window_radius + 1,
										   ...
										   z - self.window_radius : z + self.window_radius + 1]
		transpose_inds = [0,1,2]
		transpose_inds = np.random.shuffle(transpose_inds)
